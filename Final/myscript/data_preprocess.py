...
def parse_behaviors(source, target, user2int_path):
    ...
    print(behaviors.impressions[0])
    behaviors.impressions = behaviors.impressions.str.split(" ")
    print(behaviors.impressions)
    user2int = {}
    ...
    for row in behaviors.itertuples():
        positive = [x for x in row.impressions]

        positive=iter(positive)
        pairs = []
        try:
            while True:
                pair = [next(positive)]
    ...
    print(behaviors["impressions"].map(
...
def parse_news(source, target, roberta_output_dir, category2int_path,
    ...
    roberta = RobertaModel.from_pretrained('roberta-base',
                                           return_dict=True).to(torch.cuda.current_device())
